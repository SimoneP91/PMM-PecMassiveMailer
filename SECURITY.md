...
| Poison messages       | An unreadable message goes to the dead-letter queue; a message that keeps killing its container is dead-lettered by RabbitMQ's delivery limit, at least once (it leaves the input queue only once the dead-letter queue has it); a failed handling returns the message after a pause, so nothing spins. |
...
| Account lockout       | A refused login (SMTP, the receipts folder, the Sent copy) suspends the mailbox at once, sending and reading, instead of retrying; it resumes only with a restart.                                                                                                                                      |
