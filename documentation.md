...
5. **RabbitMQ in Docker keeps its data under its node name**, `rabbit@<host name>`, and a container's host name defaults to its id: a recreated container (a new image, a changed setting) finds an empty node and the queues look gone. Seen on 4.3.6: the local stack's data sits under `rabbit@9f310f781801`. [deploy/docker-compose.yml](deploy/docker-compose.yml) sets `hostname: rabbitmq`, and a PEC queued before recreating the container was still there after. The local and test stacks do not, which is harmless there. Related: a `docker exec ... rabbitmq-diagnostics` run as root in the first second of a new broker creates `.erlang.cookie` owned by root, and the broker then fails with `eacces`; the compose health checks start late enough not to.

6. **A per-tenant RabbitMQ user does not isolate publishing.** PECs go through the default exchange, and RabbitMQ checks the write permission on the exchange (`amq.default`), not on the queue it routes to. Tried on 4.3: a user with configure and read limited to `^pecmailer.serfin..*` and write on `amq.default` cannot declare or read another tenant's queues, but can publish into its input queue. Harmless with one tenant per broker; see What's Next.

---

## Deployment Notes

- **Docker Compose on one server** ([deploy/](deploy/README.md), procedure in English and Italian): RabbitMQ 4.3 and one container per mailbox, settings and secrets from a `.env` next to the compose file (mode 600), the pecmailer image pulled from the company's container registry, named with its version in `.env` (`PECMAILER_IMAGE`): the server needs the `deploy/` folder only and builds nothing, a version is published once and never as `latest`, and going back means naming the previous one. What the local stack lacks, on purpose: `stop_grace_period: 2m`, `restart: unless-stopped` on every service (after a Docker restart the local stack brings back its pecmailer containers but not RabbitMQ), `hostname: rabbitmq` (quirk 5), `read_only: true`, rotated `json-file` logs, the management page and the probes on 127.0.0.1 only. Smoke-tested with Greenmail in place of the provider: start, a PEC sent and archived, an orderly stop, a queued PEC surviving a recreated broker; with a registry of its own, the image pulled, a move to the next version and back. A changed `.env` needs `docker compose up -d`: `restart` keeps the old values. `RABBITMQ_DEFAULT_USER/PASS` count only when the broker first creates its data.
...
- **Hand-over to infrastructure**: done for one server with Docker Compose ([deploy/](deploy/README.md)). For Kubernetes: manifests (ConfigMap, Secret and Deployment per mailbox). Either way: logs to Graylog, alerts on `mailbox.suspended` and on messages in `.dead`.
- **Tenant isolation on a shared broker**: publishing through the default exchange cannot be limited per tenant (Known Quirks, 6). Before two tenants share a RabbitMQ: a vhost per tenant, or an exchange per tenant that the input queues are bound to, with the contract and the CRM guide changed accordingly.
